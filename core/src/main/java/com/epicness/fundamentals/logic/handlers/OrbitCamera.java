...
public class OrbitCamera extends GestureDetector {
    /**
    ...
    protected OrbitCamera(final CameraGestureListener gestureListener, final Camera camera) {
    ...
    public OrbitCamera(final Camera camera) {
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        return super.touchDown(screenX,
            screenY,
            pointer,
            button) || (activateKey == 0 || activatePressed);
    }
    ...
    protected boolean process(float deltaX, float deltaY, int button) {
        if (button == rotateButton) {
            ...
            float dx = deltaX * -rotateAngle;
            float dy = deltaY * rotateAngle;
            yaw += dx;
            pitch += dy;
            if (pitch > MAX_PITCH) {
                pitch = MAX_PITCH;
            ...
            camera.rotateAround(target, tmpV1.nor(), dy);
            camera.rotateAround(target, Vector3.Y, dx);
